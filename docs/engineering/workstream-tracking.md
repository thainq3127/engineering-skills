...
Workstream Tracking coordinates one durable objective across a canonical GitHub root issue, one persistent local worktree and branch, active Issues or Pull Requests, Project state, and handoffs between agents. It also provides the idempotent provisioning primitive used by `workstream-bootstrap` from a stable control checkout.
...
Run [setup-matt-pocock-skills](https://aihero.dev/skills-setup-matt-pocock-skills) first. The repository must contain `docs/agents/issue-tracker.md` and `docs/agents/workstreams.md` describing tracker access, Project identity, bootstrap checkout, worktree layout, operators, labels, and status mapping.

## Bootstrap boundary

[workstream-bootstrap](https://aihero.dev/skills-workstream-bootstrap) may call `ensure` before a Workstream claim exists because it is creating the Workstream itself. That exception is narrow: the human must have approved the complete provisioning packet, Codex must be in the configured bootstrap checkout, and the resulting root remains queued and `Unassigned` until a later ChatGPT session actually claims planning.
