...
It **does no work itself**. It doesn't bootstrap a Workstream, grill, write a spec, or fix anything — it only orients. It exists for the **user-invoked** skills above all: nothing fires those for you, so *you* have to remember they exist, and `ask-matt` is the memory you offload that to. It also points at the model-invoked disciplines that run underneath the flows — `/tdd`, `/diagnosing-bugs`, `/prototype`, `/review-composer`, `/review-synthesizer`, `/code-review`, `/workstream-tracking`, and the vocabulary references `/domain-modeling` and `/codebase-design`. It answers "which one, and when", then hands you off to the skill that actually does the job.
...
The setup and planning boundaries are durable too: repository setup is committed before a bootstrap worktree is created, and tracked Grill or Wayfinder sessions checkpoint each confirmed decision plus the exact next open question before continuing the interview.

The entry fork is explicit: repository-wide configuration goes to [setup-matt-pocock-skills](https://aihero.dev/skills-setup-matt-pocock-skills), while each new durable multi-session objective starts with [workstream-bootstrap](https://aihero.dev/skills-workstream-bootstrap) in Codex. A one-ticket change does not earn a Workstream merely because it has a branch.

The review fork is explicit: focused PRs, single-ticket changes, and narrow domain slices go to [code-review](https://aihero.dev/skills-code-review); large cumulative ranges, multi-ticket corrections, and multi-domain or cross-cutting batches go to [review-composer](https://aihero.dev/skills-review-composer), which freezes the Delivery Context already confirmed in the spec. Completed composer children go to [review-synthesizer](https://aihero.dev/skills-review-synthesizer) for product-aware finding dispositions and a second human gate that approves finding treatment, not product direction. A bug without a known cause goes to [diagnosing-bugs](https://aihero.dev/skills-diagnosing-bugs) before anyone invents a correction.
