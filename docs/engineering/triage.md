...
Triage classifies each incoming Issue as part of an existing Workstream, deserving a new Workstream after maintainer approval, or standalone. Existing members are normalized with root linkage and labels. A new Workstream receives one root, one persistent local worktree and branch, and Project registration. Standalone Issues do not get ceremonial tracking structure.
